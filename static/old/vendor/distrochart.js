...
function makeDistroChart(settings) {
  ...
  chart.renderViolinPlot = function(options) {
    ...
    let vOpts = chart.violinPlots.options;
    ...
    chart.violinPlots.update = function() {
      ...
      for (cName in chart.groupObjs) {
        ...
        if (vOpts.clamp == 0 || vOpts.clamp == -1) { //
          ...
          if (vOpts.clamp == -1) {
            ...
            if (!interpolateMax) {
              let interMaxY = kdeTester(chart.groupObjs[cName].metrics.max);
              let interMaxX = chart.groupObjs[cName].metrics.max;
            ...
            if (!interpolateMin) {
              let interMinY = kdeTester(chart.groupObjs[cName].metrics.min);
              let interMinX = chart.groupObjs[cName].metrics.min;
              var count = 25;  // Arbitrary limit to make sure we don't get an infinite loop
          ...
          if (!vOpts._yDomainVP) {
            vOpts._yDomainVP = chart.range.slice(0);
          }
      ...
      for (cName in chart.groupObjs) {
        ...
        let objBounds = getObjWidth(vOpts.width, cName);
        let width = (objBounds.right - objBounds.left) / 2;
  ...
  chart.renderBoxPlot = function(options) {
    ...
    let bOpts = chart.boxPlots.options;
    ...
    chart.boxPlots.update = function() {
      ...
      for (cName in chart.groupObjs) {
        ...
        // Get the box width
        let objBounds = getObjWidth(bOpts.boxWidth, cName);
        let width = (objBounds.right - objBounds.left);

        let sMetrics = {}; // temp var for scaled (plottable) metric values
        for (let attr in chart.groupObjs[cName].metrics) {
          sMetrics[attr] = null;
  ...
  chart.renderNotchBoxes = function(options) {
    ...
    let nOpts = chart.notchBoxes.options;
    ...
    chart.notchBoxes.update = function() {
      ...
      for (cName in chart.groupObjs) {
        ...
        // Get the box size
        let boxBounds = getObjWidth(nOpts.boxWidth, cName);
        let medianBounds = getObjWidth(nOpts.medianWidth, cName);
  ...
  chart.renderDataPlots = function(options) {
    ...
    chart.dataPlots.update = function() {
      ...
      if (chart.dataPlots.objs.g) {
        let halfBand = chart.xScale.rangeBand() / 2; // find the middle of each band
        for (let cMetric in chart.dataPlots.objs.lines) {
      ...
      for (cName in chart.groupObjs) {
        ...
        if (cPlot.objs.points) {
          if (dOpts.plotType == 'beeswarm') {
            let swarmBounds = getObjWidth(100, cName);
            let yPtScale = chart.yScale.copy()
              .range([Math.floor(chart.yScale.range()[0] / dOpts.pointSize), 0])
              .interpolate(d3.interpolateRound)
              .domain(chart.yScale.domain());
            let maxWidth = Math.floor(chart.xScale.rangeBand() / dOpts.pointSize);
            let ptsObj = {};
            let cYBucket = null;
            //  Bucket points
            ...
            //  Plot buckets
            let rightMax = Math.min(swarmBounds.right - dOpts.pointSize);
            for (let row in ptsObj) {
              let leftMin = swarmBounds.left + (Math.max((maxWidth - ptsObj[row].length) / 2, 0) * dOpts.pointSize);
              let col = 0;
