...
function createGraph(svgRoot: any, dpsByRole: {[role: string]: Array<number>}) {
  const width = 30 * Object.keys(dpsByRole).length;
  const height = 400;
  ...
  const labels = Object.keys(dpsByRole).map(role => {
    return role
      .replace(/Condition/, 'C')
      .replace(/Boon/, 'B')
      .replace(/Heal/, 'H')
  ...
  const bins: Array<Array<number>> = Object.values(dpsByRole);
